...
class smartthingstv:
  ...
  def device_unmute(self):
      cmdurl = requests.post(API_COMMAND,data=COMMAND_UNMUTE ,headers=REQUEST_HEADERS)
      self._muted = False

  def device_volume_up(self):
      cmdurl = requests.post(API_COMMAND,data=COMMAND_VOL_UP ,headers=REQUEST_HEADERS)

  def device_volume_down(self):
      cmdurl = requests.post(API_COMMAND,data=COMMAND_VOL_DOWN ,headers=REQUEST_HEADERS)

  def device_set_volume(self):
      cmdurl = requests.post(API_COMMAND,data=COMMAND_SET_VOLUME ,headers=REQUEST_HEADERS)

  def device_play(self):
      cmdurl = requests.post(API_COMMAND,data=COMMAND_PLAY ,headers=REQUEST_HEADERS)

  def device_pause(self):
      cmdurl = requests.post(API_COMMAND,data=COMMAND_PAUSE ,headers=REQUEST_HEADERS)

  def device_stop(self):
      cmdurl = requests.post(API_COMMAND,data=COMMAND_STOP ,headers=REQUEST_HEADERS)

  def device_rewind(self):
      cmdurl = requests.post(API_COMMAND,data=COMMAND_REWIND ,headers=REQUEST_HEADERS)

  def device_fast_forward(self):
      cmdurl = requests.post(API_COMMAND,data=COMMAND_FAST_FORWARD ,headers=REQUEST_HEADERS)
